...
class Box(Clutter.Actor, Clutter.Container):
    ...
    def add(self, *new_elements):
        for new_ele in new_elements:
            if 'name' not in new_ele or 'object' not in new_ele:
                raise KeyError(
                    'Cannot add element to box. Element to add must be a dict with at least "name" and "object" in his keys')
    ...
    def do_get_preferred_width(self, for_height):
        ...
        if self._horizontal:
            for element in self.elements:
                ...
                if element.get('keep_ratio') and obj_width != 0 and element.get('expand') and for_height != -1:
            ...
            # find size available for special elements with expand and
            # keep_ratio
            special_elements = list()
            special_height = inner_height
            for element in self.elements:
                # take preferred size for resizable elements in preferred size
                # calcul
                if element.get('resizable', 0) != 0:
                    pass
                else:
                    if element.get('expand') and element.get('keep_ratio'):
                        special_elements.append(element)
                    else:
                        special_height -= element[
                            'object'].get_preferred_size()[3]
            ...
            for element in self.elements:
                ...
                if element.get('resizable', 0) != 0:
                    ...
                    if element.get('keep_ratio') and original_height != 0 and for_height != -1:
                ...
                obj_height = element['object'].get_preferred_size()[3]
                if element.get('expand') and element.get('keep_ratio') and obj_width != 0 and obj_height != 0:
                    ratio = float(obj_width) / float(obj_height)
        ...
        if self._horizontal:
            ...
            for element in self.elements:
                ...
                if element.get('resizable', 0) != 0:
                    ...
                    if element.get('expand') and element.get('keep_ratio'):
                        ...
                        special_width -= element[
                            'object'].get_preferred_size()[2]
            ...
            for element in self.elements:
                ...
                if element.get('resizable', 0) != 0:
                    ...
                    if element.get('keep_ratio') and original_width != 0 and for_width != -1:
            ...
            for element in self.elements:
                obj_height = element['object'].get_preferred_size()[3]
                used_height = obj_height
                if element.get('keep_ratio') and obj_height != 0 and element.get('expand') and for_width != -1:
                    obj_width = element['object'].get_preferred_size()[2]
                    ratio = float(obj_width) / float(obj_height)
    ...
    def do_allocate(self, box, flags):
        ...
        # ignoring elements with resizable
        special_elements = list()
        special_width = inner_width
        special_height = inner_height
        needed_width = 0
        ...
        for element in special_elements:
            ...
            if self._horizontal:
                ...
                if element.get('keep_ratio') and original_width != 0 and original_height != 0:
                    factor = float(obj_width) / float(original_width)
        ...
        for element in self.elements:
            if element.get('resizable', 0) != 0:
                original_width, original_height = element[
                    'object'].get_preferred_size()[2:]
                if self._horizontal:
                    if element.get('keep_ratio') and original_width != 0:
                        obj_width = element['resizable'] * resizable_width
                        ...
                        if obj_height > inner_height:
                            ...
                            element['resizable'] = float(
                                obj_width / resizable_width)
                else:
                    if element.get('keep_ratio') and original_height != 0:
                        obj_height = element['resizable'] * resizable_height
                ...
                if self._horizontal:
                    ...
                    if element.get('keep_ratio') and original_width != 0 and original_height != 0:
                        ...
                        if obj_width > special_width:
                            obj_width = int(special_width)
                            special_width = 0
                            ratio = float(original_width) / float(
                                original_height)
                            obj_height = int(float(obj_width) / ratio)
                        else:
                            special_width -= obj_width
        ...
        if self.background:
            if self.bg_ignore_allocation_box and len(self.elements) > 0:
                if self._horizontal:
                    ...
                    bgbox.x2 = self._margin.x + \
                        self._padding.x + round(x) - self._spacing.x
                    ...
                    bgbox.y2 = self._margin.y + \
                        self._padding.y + round(y) - self._spacing.y
                    self.background.allocate(bgbox, flags)
            else:
                bgbox = Clutter.ActorBox()
                bgbox.x1 = self._margin.x
    ...
    def do_paint(self):
        ...
        for element in self.elements:
            ...
            if draw_last:
                ...
                draw_last_objects.append(
                    dict(obj=element['object'], value=draw_last_value))
            else:
