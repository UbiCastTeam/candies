...
class ClassicButton(TextContainer):
    ...
    def __init__(self, label=' ', margin=0, padding=6, texture=None, rounded=True, crypted=False):
        TextContainer.__init__(
            self, label, margin=margin, padding=padding, texture=texture, rounded=rounded, crypted=crypted)
...
class ImageButton(ClassicButton):
    ...
    def __init__(self, label=' ', image_src=None, margin=0, padding=10, spacing=10, texture=None, has_text=True, expand=False):
        ClassicButton.__init__(
            self, label, margin=margin, padding=padding, texture=texture)
    ...
    def do_allocate(self, box, flags):
        ...
        # allocate image
        if self._has_text:
            label_height = ClassicButton.do_get_preferred_height(
                self, for_width=inner_width)[1]
        ...
        if image_preferred_size[3] > 0:
            image_ratio = float(image_preferred_size[2]) / float(
                image_preferred_size[3])
            if self._expand:
                ...
                if image_width > inner_width:
                    ...
                    image_height = round(
                        float(image_width) / float(image_ratio))
            else:
...
if __name__ == '__main__':
    ...
    if test_memory_usage:
        ...
        # texture_path =
        # '/home/sdiemer/sources/candies/main/candies2/effect_light.png'
        texture = Clutter.cogl.texture_new_from_file(
            texture_path, Clutter.cogl.TEXTURE_NO_SLICING, Clutter.cogl.PIXEL_FORMAT_ANY)
        texture = None
