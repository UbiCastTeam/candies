...
class VideoPlayer(ClutterGst.VideoTexture):
    ...
    def add_listener(self, event_name, function):
        if event_name not in self._listeners:
            raise Exception(
                'Can not add function to listeners. %s is not a valid event name.' % event_name)
    ...
    def remove_listener(self, event_name, function):
        if event_name not in self._listeners:
            raise Exception(
                'Can not remove function from listeners. %s is not a valid event name.' % event_name)
    ...
    def seek_to_percent(self, percent):
        ...
        if self._seeking_timeout_id is None:
            self._seeking_timeout_id = GObject.timeout_add(
                100, self._execute_seek_request)
...
if __name__ == '__main__':
    ...
    def test(source, event, p):
        p.set_filename(
            '/home/aviolo/src/common/easycast/unstable/easycast/data/userdata/videos/test.mp4')
        p.set_audio_volume(0.4)
        p.play()

        GObject.timeout_add(
            1000, p.set_file, '/home/aviolo/src/common/easycast/unstable/easycast/data/userdata/videos/test.mp4')
        ...
        GObject.timeout_add(
            6000, p.set_file, '/home/aviolo/src/common/easycast/unstable/easycast/data/userdata/videos/test.mp4')
