...
class TexturedBlock(BaseContainer):
    ...
    def do_get_preferred_width(self, for_height):
        ...
        if self.content_actor:
            max_width = max(
                max_width, self.content_actor.get_preferred_width(h)[1])
    ...
    def do_get_preferred_height(self, for_width):
        ...
        if self.title_actor:
            ...
            preferred_height += self.default_title_actor.get_preferred_height(
                w)[1]
            if self.content_actor:
                preferred_height += self._spacing.y
        if self.content_actor:
    ...
    def do_allocate(self, box, flags):
        ...
        if self._has_title:
            self._title_height = selected_title_actor.get_preferred_height(
                for_width=inner_width)[1]
            title_box = Clutter.ActorBox()
            title_box.x1 = self._margin.x + \
                self._padding.x + self._title_padding.x
            title_box.y1 = self._margin.y + \
                self._padding.y + self._title_padding.y
            title_box.x2 = self.width - self._margin.x - \
                self._padding.x - self._title_padding.x
    ...
    def _paint_background(self):
        if self.width > 0 and self.height > 0:
            ...
            title_x2 = self._margin.y + \
                self._padding.x + self._title_textures_size
            title_x3 = self.width - self._margin.x - \
                self._padding.x - self._title_textures_size
            title_x4 = self.width - self._margin.x - self._padding.x
            title_y1 = self._margin.y + self._padding.y
            title_y2 = self._margin.y + self._padding.y + \
                2 * self._title_padding.y + self._title_height
            # title_left texture
            if self._title_left:
...
def tester(stage):
    ...
    """
    textures_package = {
        'top_left': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'top_left', '.png')),
        'top': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'top', '.png')),
        'top_right': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'top_right', '.png')),
        'middle_left': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'middle_left', '.png')),
        'middle': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'middle', '.png')),
        'middle_right': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'middle_right', '.png')),
        'bottom_left': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'bottom_left', '.png')),
        'bottom': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'bottom', '.png')),
        'bottom_right': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'bottom_right', '.png')),

        'title_left': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'title_left', '.png')),
        'title_middle': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'title_middle', '.png')),
        'title_right': Clutter.Texture.new_from_file('%s%s%s' %(textures_path, 'title_right', '.png')),
    }
    """
    ...
    # global_bg = TexturedBlock('Nouveau block', title_actor=title,
    ...
    if test_memory_usage:
        ...
        def create_test_object(textures_package):
            ...
            t = TexturedBlock(
                'Nouveau block', content=content, textures_package=textures_package)
            t.set_size(800, 300)
            return t
        ...
        def remove_tested_object(tested_object, stage, counter, textures_package):
            ...
            GObject.timeout_add(
                2, test_memory, stage, counter, max_count, textures_package)
            return False

        GObject.timeout_add(
            10, test_memory, stage, 0, max_count, textures_package)
